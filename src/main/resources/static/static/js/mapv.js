    ...
    draw: function draw(context, dataSet, options) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (item.properties && item.properties.cluster) {
                ...
                if(pointCountMin == pointCountMax) pointCountMax++
            }
        }
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            if (item.properties && item.properties.cluster) {
                ...
                // context.fillStyle = item.properties.point_count == 1 ? data[i].geometry.fillStyle : intensity.getColor(item.properties.point_count);
                context.fillStyle = '#5ABA94';
                context.fill();
                ...
                if(item.geometry.data.is_virtual == 0){
                    context.rect(coordinates[0]-5, coordinates[1]-5, 10, 10);
                }else{
                    context.arc(coordinates[0], coordinates[1], options.size || 5, 0, Math.PI * 2);
                }
                // context.fillStyle = options.fillStyle || 'red';
                // context.fillStyle = data[i].geometry.fillStyle || options.fillStyle || 'red';
                if(data[i].geometry.data[5]=="1"){
                    context.fillStyle = '#E8684A';
                }else if(data[i].geometry.data[5]=="2"){
                    context.fillStyle = '#FF9D4D';
                }else if(data[i].geometry.data[5]=="3"){
                    context.fillStyle = '#5D7092';
                }else if(data[i].geometry.data[5]=="4"){
                    context.fillStyle = '#5B8FF9';
                }else{
                    context.fillStyle = '#5ABA94';
                }
                // context.fillStyle = data[i].geometry.data.handleState || options.fillStyle || 'red';
                context.fill();
            }
        }
        context.restore();
...
var Layer = function (_BaseLayer) {
    ...
    createClass(Layer, [
        ...
        {
            key: "make_superData",
            value: function make_superData(dataSet) {
                this.supercluster = new Supercluster({ maxZoom:19, radius: 150 });
                this.supercluster.load(dataSet.get());
                console.log(this);
                this.clusterDataSet = new DataSet();
            }
